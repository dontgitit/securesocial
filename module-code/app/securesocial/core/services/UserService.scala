...
trait UserService[U] {
   ...
   */
  def find(providerId: String, userId: String): Future[Option[U]]
   ...
   * @return an optional profile
   */
  def findByEmailAndProvider(email: String, providerId: String): Future[Option[U]]
   ...
   */
  def save(profile: U, mode: SaveMode): Future[U]
   ...
   */
  def link(current: U, to: U): Future[U]
   ...
   * @return
   */
  def updatePasswordInfo(user: U, info: PasswordInfo): Future[Option[U]]
